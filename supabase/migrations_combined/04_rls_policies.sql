...
DROP POLICY IF EXISTS "Users can view own racha" ON rachas;
CREATE POLICY "Users can view own racha"
    ON rachas FOR SELECT
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can update own racha" ON rachas;
CREATE POLICY "Users can update own racha"
    ON rachas FOR UPDATE
    USING (auth.uid() = user_id);

-- ===========================================
-- POLÍTICAS DE CONTENIDO EDUCATIVO
-- ===========================================

DROP POLICY IF EXISTS "Authenticated users can view oposiciones" ON oposiciones;
CREATE POLICY "Authenticated users can view oposiciones"
    ON oposiciones FOR SELECT
    TO authenticated
    USING (activa = TRUE);

DROP POLICY IF EXISTS "Authenticated users can view bloques" ON bloques;
...
DROP POLICY IF EXISTS "Authenticated users can view temas" ON temas;
...
DROP POLICY IF EXISTS "Authenticated users can view subtemas" ON subtemas;
...
DROP POLICY IF EXISTS "Authenticated users can view preguntas" ON preguntas;
CREATE POLICY "Authenticated users can view preguntas"
    ON preguntas FOR SELECT
    TO authenticated
    USING (TRUE);

DROP POLICY IF EXISTS "Authenticated users can view respuestas" ON respuestas;
...
DROP POLICY IF EXISTS "Users can view own test_respuestas" ON test_respuestas;
...
DROP POLICY IF EXISTS "Users can create own test_respuestas" ON test_respuestas;
...
DROP POLICY IF EXISTS "Users can view own user_oposiciones" ON user_oposiciones;
CREATE POLICY "Users can view own user_oposiciones"
    ON user_oposiciones FOR SELECT
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can manage own user_oposiciones" ON user_oposiciones;
CREATE POLICY "Users can manage own user_oposiciones"
    ON user_oposiciones FOR ALL
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can view own sesiones" ON sesiones_estudio;
CREATE POLICY "Users can view own sesiones"
    ON sesiones_estudio FOR SELECT
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can manage own sesiones" ON sesiones_estudio;
CREATE POLICY "Users can manage own sesiones"
    ON sesiones_estudio FOR ALL
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can view own metricas" ON metricas_diarias;
CREATE POLICY "Users can view own metricas"
    ON metricas_diarias FOR SELECT
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can view own progreso_temas" ON progreso_temas;
CREATE POLICY "Users can view own progreso_temas"
    ON progreso_temas FOR SELECT
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can manage own progreso_temas" ON progreso_temas;
...
DROP POLICY IF EXISTS "Users can view own alertas" ON alertas_abandono;
CREATE POLICY "Users can view own alertas"
    ON alertas_abandono FOR SELECT
    USING (auth.uid() = user_id);

DROP POLICY IF EXISTS "Users can view own intervenciones" ON intervenciones;
...
DROP POLICY IF EXISTS "Users can view own flashcard_reviews" ON flashcard_reviews;
...
DROP POLICY IF EXISTS "Users can create own flashcard_reviews" ON flashcard_reviews;
...
DROP POLICY IF EXISTS "Everyone can view logros" ON logros;
CREATE POLICY "Everyone can view logros"
    ON logros FOR SELECT
    TO authenticated
    USING (TRUE);

DROP POLICY IF EXISTS "Users can view own user_logros" ON user_logros;
